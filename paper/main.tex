...
\author[1,2]{Fabian Hofmann}
\author[1]{Markus Schlott}

\affil[1]{Frankfurt Institute for Advanced Studies (FIAS), 60438 Frankfurt, Germany}
\affil[2]{Technical Univerisity Berlin, Department of Energy Systems, 10623 Berlin}
...
\twocolumn[
    \begin{@twocolumnfalse}
        ...
        \begin{abstract}
            Optimizing the total cost of power systems is a common tool for network operation and planning. Besides valuable information about how to run and possibly expand the power system, the optimization yields an optimal Locational Marginal Price per node and time step. That is, the price of electricity paid by consumers and purchased by suppliers while maximizing the social welfare. Naturally, these prices are a direct result of the optimization problem, so do not give any information about their composition.

            This paper shows that by applying Flow Tracing, an algorithm developed for transmission systems, it is possible to interlink the Locational Marginal Prices in a coherent mathematical manner. This not only leads to important insights into the price structure but enables to an make an intuitive decomposition and allocation of all costs. Therefore, individual consumers "see" how much they pay to individual generators and transmission lines. This method, which we introduce as Price Tracing, outperforms similar approaches provided by the literature as resulting cost allocations are transparent, plausible, and consistent with Locational Marginal Prices. The Price Tracing method is applied and discussed on behalf of a power system model of Germany with a high share of renewables. The presented analysis can help in finding a more efficient market design to drive renewable power supply.
            % \vtop{%
            %  \vskip0pt
            %   \hbox{%
