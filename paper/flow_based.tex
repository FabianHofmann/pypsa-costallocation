...
\usepackage[natbib=true, sorting=none, backend=biber, bibencoding=utf8]{biblatex}
\usepackage{eurosym}
...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    \begin{abstract}
        Maximizing the welfare of all market participants within a power system is a common approach in energy system modelling. It leads to perfectly scheduled operations and investments while securing all power demands. \dots 
        This work introduces a peer-to-peer cost allocation for welfare-maximized systems   \dots
        This opens the possibility to transparently integrate transmission constraints and investment costs in national and international development plans.
    \end{abstract}
...
\begin{itemize}
    \item In long-term investment models every network asset gains back its variable and investment cost from its operation based revenue
    \item Flow tracing is used as a basis to allocate the operation of assets to consumers. 
    \item Using operational and shadow prices, the allocations assign all system costs in an P2P manner.
    ...
    \item Applying to a German future scenario shows how higher electricity costs arise for buses relying on investments in transmission and local backup generation  
\end{itemize}
...
Applied to a optimized German power system with an imposed price of 120 \euro\, per tonne CO$_2$ equivalent, the cost allocation shows how 

The presented cost allocation is based on the linear power flow approximation. Yet, the method is equally applicable to a system with an Optimal Power Flow (OPF), \ie full AC power flows. However, the AP scheme might not the right choice as the Kirchhoff Voltage Law cannot not be considered subsequently as done here in the linear case. Rather, the Z-Bus flow allocation presented in \cite{conejo_z-bus_2007} might be a better choice as it considers by default the two circuit laws. The full power flow introduces an additional cost term $\remainingcost^\text{Loss}$ accounting for the transmission loss which is compensated by the consumers and mirrors in the LMP. 
...
In the presented work, we restricted the application to long term investment models with perfect foresight. However, the cost allocation can as well be applied to short term planning models with fixed capacities. The revenue from the capacity limits then builds basis for future investments.
