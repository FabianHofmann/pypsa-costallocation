...
\end{figure}
We impose a effective carbon price of 120 \euro\, per tonne-CO$_{2}$ which, with an gross emission of 180~kg/MWh and an efficiency of 39\% for OCGT, adds an effective price of 55 \euro/\megawatthour. All cost assumptions on operational costs $o_i$ and annualized capital cost $c_i$ are summarized in detail in \cref{tab:cost_assumptions}. 
...
We allocate all costs under the assumption that the \textit{net} dispatch is assigned according to the AP method (type \ref{net}\ref{ap}). 
...
In \cref{fig:capex_price} we depict the share of the average nodal price $\left<\lmp\right>_t$ which is dedicated to the CAPEX of different technologies. It turns out that  mostly areas with high CAPEX allocation are the ones with low average LMP. In particular, this counts for the coastal regions where strong offshore and onshore investments are taken. Inside the country, some regions also pay a fairly high shares dedicated to CAPEX, which are however allocated to non-local assets. The most even spatial distribution of CAPEX allocations accounts for OCGT. These are however primarily built nearby strong wind capacities in the North.  
...
The last cost term in \cref{fig:total_cost}, is  caused by lower capacity constraints for pre-existing assets. These violate the optimal design and are not compensated by the nodal payments $\demand \, \lmp$. This ...
