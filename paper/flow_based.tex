...
In a long-term equilibrium of a power system with perfect competition and no further constraints, the zero-profit condition states that each cost term $\cost_{i}$ is recovered by the revenue that asset $i$ receives from the market \cite{steiner_peak_1957}. As partly shown in \cite{brown_decreasing_2020}, it builds on the fact that $\cost_i$ can be expressed as a cost-weighted sum of the operational state $s_{i,t}$ of asset $i$ and time $t$, \ie
\begin{align}
