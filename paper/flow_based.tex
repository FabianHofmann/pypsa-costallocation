...
In more realistic setups, generators, lines or other assets can only be built up to a certain limit. This might caused by land use restrictions or social acceptance problems. %In the following give a general solution how the cost allocation may handle this.  
...
\end{align}
the zero profit condition for asset $i$ alters. Then the revenues don't match the expenditures as soon as the constraint becomes binding. More precisely, the revenue from the shadow prices given in \cref{tab:cost_allocation_map} exceed the CAPEX $\cost_i$ by $\remainingcost_i$,  where  $\remainingcost_i$ is given by
\begin{align}
...
% \end{align}
For the cost allocation, these means the prices in \cref{tab:cost_allocation_map} have to be adjusted, as otherwise consumers would pay too much. As we show in ... updating the prices by a fix ratio 
\begin{align}
    \costfactor \rightarrow \dfrac{c_i}{c_i + \muuppernom}\, \costfactor
    \label{eq:adjustment_lowernom}
\end{align}
will ensure that the allocated CAPEX sum up to the actual CAPEX at asset $i$, meaning that \cref{eq:cost_payback} holds. Note that $\muuppernom$ is a positive number, so the adjustment cannot increase the paid price. 
...
In that case, asset $i$ does not collect enough revenue in order to match the CAPEX. The difference is given by 
\begin{align}
...
We apply the cost allocation on a German power system model 
