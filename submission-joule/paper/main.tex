...
\author{Fabian Hofmann\corref{cor1}}
\ead{hofmann@fias.uni-frankfurt.de}
\address{Frankfurt Institute for Advanced Studies (FIAS), 60438 Frankfurt, Germany}
...
\begin{highlights}
    \item Expenditures to a network asset are recovered by  its operation based revenue
    \item Flow tracing is used to allocate the operation of assets to consumers 
    \item Allocated flows have to be reshaped to meet the Kirchhoff circuit laws
    \item Using operational and shadow prices, all system costs are assigned in an P2P manner
    \item Cost assignments are local while aliging with the nodal pricing scheme of the optimum    
...
\begin{keyword}
power system modelling; economic optimization; cost allocation; investment planning; benefit analysis
\end{keyword}
...
The first section presents the zero-profit condition for different kind of assets (\ref{sec:zero_profit_rule}),  the AP allocation scheme and derived allocations from asset to consumer (\ref{sec:dispatch_allocations}), the cost allocation and the impact of additional constraints (\ref{sec:general_scheme,sec:design_constraints}), a numerical example (\ref{sec:numerical_example}). \ref{sec:application_case} applies the cost allocation to a optimized German power system with a high share of renewable power generation and evaluates the allocated costs.  
...
\begin{table*}[t]
    ...
    \end{tabular}
    \end{center}
    \caption{Representation of different cost terms as a function of the operational state, \ie matching the form in \cref{eq:cost_decomposition}. These include OPEX \& CAPEX for production, transmission and storage assets in the network, as well as a cost term for the total Green House Gas (GHG) emissions. For storage units, an additional cost term adds to $\capexstorage$ (see \ref{sec:zero_profit_storage_units} for details).}
...
\end{align}
At the cost-optimum, the two constraints yield the shadow prices $\muupperflow$ and $\mulowerflow$. Again, we use the relation that over the whole time span, the shadow prices weighted by the flow match the investment in line $\ell$ (for details see \ref{sec:zero_profit_flow}) 
\begin{align}
...
Let $R \subset I$ denote all storages in the system. In a simplified storage model, $\capacitystorage$ limits the storage dispatch $\storagedispatch$ and charging $\storagecharge$. Further it limits the maximal storage capacity $\storagesoc$ by a fix ratio $h_r$, denoting the maximum hours at full discharge. The storage $r$ dispatches power with efficiency $\efficiencydispatch$, charges power with efficiency $\efficiencycharge$ and preserves power from one time step $t$ to the next, $t+1$, with an efficiency of $\efficiencysoc$. In \ref{sec:zero_profit_storage_units} we formulate the mathematical details. The OPEX which adds to $\totalcost$ is given by 
\begin{align}
...
Requests for further information, resources and materials should be directed to Fabian Hofmann (hofmann@fias.uni-frankfurt.de)

\subsection*{Data and Code Availability}
All figures and data points of this paper can be reproduced by using the \textit{python} workflow published in \cite{hofmann_pypsa-costallocation_2020}. \\
The automated workflow allows for higher spatial resolution of the network (scalable up to the full ENTSO-E Transmission System Map) and optionally taking the total European power system into account.  
...
This work was solely done by Fabian Hofmann.  
...
\begin{figure*}
    \includegraphics[width=.9\linewidth]{de50bf/capex_duration_curve}
